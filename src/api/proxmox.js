...
export class Client {
  ...
  async listPoolMembers(poolid, type) {
    const {
      data: { data },
    } = await this.axios.get("/pools", { params: { poolid, type } });
    return data;
  }
  ...
  async taskStatus(node, upid) {
    const {
      data: { data },
    } = await this.axios.get(`/nodes/${node}/tasks/${upid}/status`);
    return data;
  }
  ...
  async qemuClone(node, vmid, newid, options) {
    const {
      data: { data },
    } = await this.axios.post(`/nodes/${node}/qemu/${vmid}/clone`, {
      name: options.name,
  ...
  async qemuConfig(node, vmid) {
    const {
      data: { data },
    } = await this.axios.get(`/nodes/${node}/qemu/${vmid}/config`);
    return data;
  }
  ...
  async qemuSetConfig(node, vmid, config = {}) {
    const {
      data: { data },
    } = await this.axios.put(`/nodes/${node}/qemu/${vmid}/config`, config);
    return data;
  }
  ...
  async qemuSetStatus(node, vmid, status) {
    const {
      data: { data },
    } = await this.axios.post(`/nodes/${node}/qemu/${vmid}/status/${status}`);
    return data;
  }
  ...
  async qemuDelete(node, vmid, options = {}) {
    const {
      data: { data },
    } = await this.axios.delete(`/nodes/${node}/qemu/${vmid}`, {
      params: {
